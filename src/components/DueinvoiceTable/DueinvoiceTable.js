...
import { getTotalSizeofInvoice } from 'actionStore/FirebaseAction/dueInvoiceAction';
...
const headCells = [
    { id: 'Parent_name', numeric: false, disablePadding: true, label: 'ParentName' },
    { id: 'Child_name', numeric: false, disablePadding: true, label: 'ChildName' },
    { id: 'className', numeric: false, disablePadding: true, label: 'className' },
    { id: 'dueDate', numeric: true, disablePadding: false, label: 'Due Date' },
    { id: 'dueTime', numeric: false, disablePadding: false, label: 'Due Time' },
    { id: 'isPaid', numeric: false, disablePadding: false, label: 'IsPaid' },
    // { id: 'isVerify', numeric: false, disablePadding: false, label: 'isVerify' },
];
...
export default function EnhancedTable(props) {
    ...
    const getTotalPagesCount = async () => {
        const totalPages = await getTotalSizeofInvoice();
        setCount(totalPages)
    }
    ...
    const onTogglePaid = (value) => {
        props.onTogglePaid(value)
    }

    const onToggleVerify = (value) => {
        props.onToggleVerify(value)
    }
                                ...
                                return (
                                    ...
                                    >
                                        ...
                                        <TableCell className={classes.tableCell} >
                                            <Button color="buttonGradient" className={classes.loginButton} onClick={(event) => {
                                                event.stopPropagation();
                                                event.preventDefault();
                                                onTogglePaid(row)
                                            }} >{row.is_paid ? `YES` : `NO`}</Button>
                                        </TableCell>
                                        {/* <TableCell className={classes.tableCell} >
                                            <Button color="buttonGradient" className={classes.loginButton} onClick={(event) => {
                                                event.stopPropagation();
                                                event.preventDefault();
                                                onToggleVerify(row)
                                            }} >{row.is_verify ? `YES` : `NO`}</Button>
                                        </TableCell> */}
                                        {/* <TableCell className={classes.tableCell} >{String(row.is_verify)}</TableCell> */}
                                    </TableRow>
                                );
                            })}
                      {showLoader ? (
