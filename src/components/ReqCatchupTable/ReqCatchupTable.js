...
import { getTotalSizeofReqCatchup } from 'actionStore/FirebaseAction/requestCatchupAction';
...
export default function EnhancedTable(props) {
    ...
    const getTotalPagesCount = async () => {
        const totalPages = await getTotalSizeofReqCatchup();
        setCount(totalPages)
    }
    ...
    const onToggleStatus = (selectedData) => {
        props.onToggleStatus(selectedData)
    }
                    ...
                    <TableBody>
                        {stableSort(tableData, getComparator(order, orderBy))
                            ...
                            .map((row, index) => {
                                ...
                                const t = moment(row.start_time, 'hh:mm a');
                                ...
                                return (
                                    ...
                                    >
                                        ...
                                        <TableCell className={classes.tableCell} component="th" id={labelId} scope="row" padding="none">
                                            {row.class_name == null ? '-' : row.class_name}
                                        </TableCell>
                                        ...
                                        <TableCell className={classes.tableCell} >{formateTime}</TableCell>
                                        <TableCell className={classes.tableCell} onClick={(event) => {
                                            event.stopPropagation();
                                            event.preventDefault();
                                            onToggleStatus(row)
                                        }}>
                                            <Button color="buttonGradient" className={classes.loginButton} >{row.isOpen ? `open` : `close`}</Button>
                                        </TableCell>
                                    </TableRow>
                                );
                            })}
